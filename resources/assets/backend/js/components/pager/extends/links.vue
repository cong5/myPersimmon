    ...
    <div class="pit-content">
        ...
        <template>
            <el-table :data="listData" v-loading="listLoading" style="width: 100%"
                ...
                <el-table-column prop="name" sortable min-width="300" label="网站名称"></el-table-column>
                <el-table-column prop="url" sortable min-width="300" label="网站地址">
                    <template scope="scope">
                        <a :href="scope.row.url" class="links" target="_blank"><i class="fa fa-external-link" aria-hidden="true"></i> {{scope.row.url}}</a>
                    </template>
                </el-table-column>
                <el-table-column label="Logo" width="200">
                    <template scope="scope">
                        <span v-if="scope.row.logo">
                            <img :src="scope.row.logo" style="width:100px;height: 40px;" alt="">
                        </span>
                    </template>
                </el-table-column>
            ...
            <div class="pit-dialog-edit-form" v-loading="editFormLoading">
                <el-form ref="myForm" :rules="myRules" class="myForm" label-width="100px" :model="myForm">
                    ...
                    <el-form-item label="Logo">
                        <el-input v-model="myForm.logo" auto-complete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="分组">
                        <el-input v-model="myForm.group" auto-complete="off"></el-input>
                    </el-form-item>
	...
	export default{
		...
		methods: {
			...
			handleEdit: function (row) {
				...
				_this.editFormVisible = true;
				_this.axios.get('/links/' + row.id).then(function (response) {
					let res = response.data;
					if (res != false) {
						_this.myForm = res;
					} else {
			...
			handleDistory: function (type, row) {
				...
				_this.$confirm('确认删除该记录吗?', '提示', {
					...
					_this.listLoading = true;
					_this.axios.delete('/links/destroy',{ data: idsParam }).then(function (response) {
						_this.listLoading = false;
						let res = response.data;
						_this.$message({
			...
			submitMyForm: function (myForm) {
				...
				_this.$refs[myForm].validate((valid) => {
					...
					if (_this.myForm.id > 0) {
						_this.axios.put('/links/update', _this.myForm).then(function (response) {
							let res = response.data;
							_this.$message({
